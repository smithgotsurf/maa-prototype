...
function SeasonDetail({ season, sportTypes, onSave, onCancel }: SeasonDetailProps) {
  ...
  return (
    <>
      ...
      <div className="card bg-white border border-base-300 p-5 mb-4">
        <div className="mb-3">
      ...
      <div className="card bg-white border border-base-300 p-5">
        <div className="flex justify-between items-center mb-3">
          <div className="text-[11px] font-semibold text-base-content/50 uppercase tracking-wider">Programs ({programs.length})</div>
          <button className="btn btn-outline btn-sm" onClick={addProgram}><Ic d={icons.plus} s={11} /> Add Program</button>
        </div>
        {programs.length === 0 && <p className="text-base-content/50 text-sm text-center py-4">No programs yet. Click "Add Program" to get started.</p>}
        {programs.length > 0 && (
            ...
            <table className="table table-sm">
              ...
              <tbody>{programs.map((p) => (
                <tr key={p.id}>
                  ...
                  <td><div className="flex items-center gap-0.5"><span className="text-xs text-base-content/50">$</span><input type="number" className="input input-bordered input-xs w-16" value={p.fee} onChange={(e) => updateProgram(p.id, 'fee', +e.target.value)} /></div></td>
...
export default function AdminPage() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[190px_1fr] min-h-[calc(100vh-56px)]">
      <aside className="hidden md:block bg-white border-r border-base-300 py-4.5 px-0">
        <div className="text-[11px] font-semibold text-base-content/50 uppercase tracking-wider mb-2 px-4.5">Management</div>
        {[
          { id: 'dash', ic: icons.clip, l: 'Registrations' },
          { id: 'seasons', ic: icons.gear, l: 'Seasons' },
          { id: 'users', ic: icons.users, l: 'Users' },
        ].map((i) => (
          <button key={i.id} className={`btn btn-ghost btn-sm justify-start gap-2 w-full mb-1 ${tab === i.id ? 'bg-primary/10 text-secondary font-semibold border-r-[3px] border-r-primary' : ''}`} onClick={() => sTab(i.id)}>
            <Ic d={i.ic} s={15} />{i.l}
          </button>
        ))}
      </aside>
      <main className="p-6">
        ...
        {tab === 'dash' && (
          <>
            ...
            <div className="grid grid-cols-[repeat(auto-fill,minmax(100px,1fr))] gap-2.5 mb-5">
              <div className="card bg-white border border-base-300 rounded-[10px] p-3 border-l-4 border-l-primary text-center">
                <div className="text-[10px] text-base-content/50 font-semibold uppercase tracking-wide">Total</div>
                <div className="text-[22px] font-serif text-secondary mt-0.5">{REGS.length}</div>
                <div className="text-[10px] text-base-content/30 mt-px">all programs</div>
              </div>
              {(activeSeason?.programs || []).map((p) => (
                <div className="card bg-white border border-base-300 rounded-[10px] p-3 text-center" key={p.id}>
                  <div className="text-[10px] text-base-content/50 font-semibold uppercase tracking-wide">{p.name}</div>
                  <div className="text-[22px] font-serif mt-0.5">{progCounts[p.name]}</div>
                  <div className="text-[10px] text-base-content/30 mt-px">registered</div>
                </div>
              ))}
              <div className="card bg-white border border-base-300 rounded-[10px] p-3 border-l-4 border-l-success text-center">
                <div className="text-[10px] text-base-content/50 font-semibold uppercase tracking-wide">Revenue</div>
                <div className="text-[22px] font-serif text-secondary mt-0.5">${totalRev.toLocaleString()}</div>
                <div className="text-[10px] text-base-content/30 mt-px">collected</div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 mb-4">
              <select className="select select-bordered select-xs text-xs w-auto" value={sf} onChange={(e) => sSf(e.target.value)}>
                <option>All</option>
                {[...new Set(REGS.map((r) => r.program))].map((s) => <option key={s}>{s}</option>)}
              </select>
              <select className="select select-bordered select-xs text-xs w-auto" value={stf} onChange={(e) => sStf(e.target.value)}>
                <option>All</option><option>Completed</option><option>Pending</option>
              </select>
              <input className="input input-bordered input-xs text-xs max-w-[220px]" placeholder="Search player or parent..." value={q} onChange={(e) => sQ(e.target.value)} />
              ...
              <table className="table table-sm">
                ...
                <tbody>{filt.map((r) => (
                  <tr key={r.id}>
                    ...
                    {vis('total') && <td className="font-semibold text-secondary">${r.total}</td>}
                    {vis('date') && <td className="text-base-content/30 text-[11px]">{r.date}</td>}
                  </tr>
                ))}</tbody>
              </table>
            </div>
            <p className="text-[11px] text-base-content/30 mt-2 text-right">{filt.length} of {REGS.length}</p>
            {showEquipModal && (
              <div className="modal modal-open">
                <div className="modal-box max-w-md">
                  <h3 className="text-lg font-bold mb-4">Equipment Summary</h3>
                  <div className="text-[11px] font-semibold text-base-content/50 uppercase tracking-wider mb-2">Hat Sizes</div>
                  <div className="grid grid-cols-3 gap-2 mb-4">
                    {HATS.map((h) => (
                      <div key={h} className="bg-base-200 rounded-md p-2.5 text-center">
                        <div className="text-[11px] text-base-content/50 font-semibold">{h}</div>
                        <div className="text-xl font-bold mt-0.5">{hatCounts[h] || 0}</div>
                      </div>
                    ))}
                  </div>
                  <div className="text-[11px] font-semibold text-base-content/50 uppercase tracking-wider mb-2">Jersey Sizes</div>
                  <div className="grid grid-cols-4 gap-2">
                    {JERSEYS.map((j) => (
                      <div key={j} className="bg-base-200 rounded-md p-2.5 text-center">
                        <div className="text-[10px] text-base-content/50 font-semibold">{j}</div>
          ...
          <>
            {seasonView === 'list' ? (
              <>
                ...
                {activeSeason ? (
                  <div className="card bg-white border-2 border-primary p-5 mb-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <div>
                        <h2 className="text-lg font-bold">{activeSeason.name}</h2>
                        {activeSeason.description && <p className="text-xs text-base-content/50 mt-0.5">{activeSeason.description}</p>}
                      </div>
                  ...
                  <div className="card bg-base-200 p-4 mb-4 text-sm text-base-content/50">No active season. Activate a season from the table below.</div>
                )}
                <div className="text-[11px] font-semibold text-base-content/50 uppercase tracking-wider mt-5 mb-2">All Seasons</div>
                <div className="overflow-x-auto">
                  <table className="table table-sm">
                    ...
                    <tbody>{seasons.map((s) => (
                      <tr key={s.id}>
                        ...
                        <td className="text-[11px] text-base-content/50 max-w-[200px]">{s.description || '—'}</td>
        ...
        {tab === 'users' && (
          <>
            ...
            <p className="text-base-content/50">Assign Admin and Registrar roles here. Coming soon.</p>
          </>
        )}
      </main>
    </div>

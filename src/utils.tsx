...
export function age(dob: string, asOf?: string | null): number {
  const d = ld(dob);
  const c = asOf ? ld(asOf) : new Date();
  let a = c.getFullYear() - d.getFullYear();
  if (d.getMonth() > c.getMonth() || (d.getMonth() === c.getMonth() && d.getDate() > c.getDate()))
    a--;
  return a;
}
export function fmtDate(s: string): string {
  return ld(s).toLocaleDateString();
}
export function recommended(p: Player, progs: Program[]): Program[] {
  return progs.filter((pr) => {
    const a2 = age(p.dob, pr.ageAsOfDate);
    return a2 >= pr.min && a2 <= pr.max && (pr.gender === 'Coed' || pr.gender === p.gender);
  });
}
export function otherPrograms(p: Player, progs: Program[]): Program[] {
  const rec = recommended(p, progs);
  return progs.filter((pr) => !rec.find((r) => r.id === pr.id));
}

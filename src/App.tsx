...
export default function App(){
  ...
  return(<div>
    ...
    <nav className="H-nav">{nav.map(n=>(<NavLink key={n.to} to={n.to} className={({isActive})=>isActive?"on":""}><Ic d={n.ic} s={14}/>{n.l}{n.badge&&n.badge>0&&<span className="hbadge">{n.badge}</span>}</NavLink>))}<button><Ic d={icons.user} s={14}/>Josh S.</button></nav>
    ...
    <nav className={`H-mob${menuOpen?" open":""}`}>{nav.map(n=>(<NavLink key={n.to} to={n.to} className={({isActive})=>isActive?"on":""} onClick={()=>setMenuOpen(false)}><Ic d={n.ic} s={16}/>{n.l}{n.badge&&n.badge>0&&<span className="hbadge">{n.badge}</span>}</NavLink>))}<button onClick={()=>setMenuOpen(false)}><Ic d={icons.user} s={16}/>Josh S.</button></nav>
    <Outlet/>
  </div>)}
